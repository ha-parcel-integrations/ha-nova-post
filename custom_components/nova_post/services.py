"""Services for the Nova Post parcel tracker integration.

`nova_post.track_parcel` / `nova_post.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Nova Post hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Nova Post is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Nova Post services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Nova Post tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Nova Post services (single-entry integration, so on unload)."""
