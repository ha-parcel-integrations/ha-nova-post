"""Tests for the Nova Post services (track_parcel / untrack_parcel)."""
...
async def test_track_parcel_normalizes_code(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "track_parcel",
            {CONF_TRACKING_CODE: "2045 0000-000001"},
            blocking=True,
        )
        await hass.async_block_till_done()
...
async def test_untrack_parcel_removes_from_options(hass):
    entry = await _setup(hass, parcels=[{CONF_TRACKING_CODE: CODE}])
    with patch(
        "custom_components.nova_post.api.NovaPostApiClient.async_get_parcel",
        new=AsyncMock(return_value=_SAMPLE),
    ):
...
async def test_untrack_unknown_code_is_noop(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "untrack_parcel",
            {CONF_TRACKING_CODE: "20450000000009"},
            blocking=True,
        )
        await hass.async_block_till_done()
