"""Tests for the Nova Post coordinator: fetching, caching, events and the
...
from custom_components.nova_post.api import NovaPostApiError
...
from custom_components.nova_post.coordinator import (
    NovaPostCoordinator,
...
def out_for_delivery_sample(code: str = ACTIVE_CODE, **overrides: object) -> dict:
    """Code ``101`` — "Uploaded to the courier", maps to out_for_delivery."""
    return raw_response(code, "101", "Uploaded to the courier", **overrides)
...
async def test_update_merges_multiple_parcels(hass):
    ...
    client.async_get_parcel.side_effect = lambda code: (
        in_transit_sample() if code == ACTIVE_CODE else delivered_sample()
    )
    coordinator = NovaPostCoordinator(hass, client, entry)
...
async def test_update_not_found_shows_pending_placeholder(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = None  # StatusCode 3 -> not found
    coordinator = NovaPostCoordinator(hass, client, entry)
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = NovaPostApiError("HTTP 500")
...
async def test_event_carries_device_id(hass):
    ...
    client.async_get_parcel.return_value = registered_sample()
    await coordinator._async_update_data()
    client.async_get_parcel.return_value = in_transit_sample()
    await coordinator._async_update_data()
    await hass.async_block_till_done()
...
async def test_delivery_fires_delivered_event_and_not_status_changed(hass):
    ...
    client.async_get_parcel.return_value = in_transit_sample(ACTIVE_CODE)
    await coordinator._async_update_data()
...
async def test_no_events_for_parcel_first_seen_delivered(hass):
    ...
    client = AsyncMock()
    client.async_get_parcel.side_effect = lambda code: (
        in_transit_sample(code) if code == ACTIVE_CODE else delivered_sample(code)
    )
    coordinator = NovaPostCoordinator(hass, client, entry)
...
async def test_fires_registered_event_for_new_parcel(hass):
    ...
    client.async_get_parcel.side_effect = lambda code: in_transit_sample(code)
    await coordinator._async_update_data()
    await hass.async_block_till_done()
