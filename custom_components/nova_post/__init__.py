"""Nova Post parcel tracker custom component for Home Assistant."""
...
from .api import NovaPostApiClient
from .const import PLATFORMS
from .coordinator import NovaPostCoordinator, _refresh_interval
...
@dataclass
class NovaPostData:
    """Runtime data attached to the Nova Post config entry."""

    client: NovaPostApiClient
    coordinator: NovaPostCoordinator


type NovaPostConfigEntry = ConfigEntry[NovaPostData]


async def async_setup_entry(hass: HomeAssistant, entry: NovaPostConfigEntry) -> bool:
    """Set up Nova Post from a config entry."""
    # No auth: Nova Post tracking is public, so the HA-managed session is fine.
    client = NovaPostApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = NovaPostData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: NovaPostConfigEntry) -> bool:
    """Unload the Nova Post config entry."""
