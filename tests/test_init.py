"""Tests for Nova Post setup and unload."""
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.nova_post.api.NovaPostApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.nova_post_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.nova_post.api.NovaPostApiClient.async_get_parcel",
        new=AsyncMock(side_effect=NovaPostApiError("Nova Post unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.nova_post.api.NovaPostApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
