# Automatic parcel tracking from e-mail (IMAP → Nova Post)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Nova Post tracking code, and register it with `nova_post.track_parcel` — fully automatic, no extra custom component required.

Nova Post is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `nova_post.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ nova_post.track_parcel
                                     │
...
- This integration, with the `nova_post.track_parcel` action available (field `tracking_code`).
...
The regex matches a standalone 14-digit run:

```
(?<![0-9])[0-9]{14}(?![0-9])
```

A Nova Poshta TTN is **exactly 14 digits, with no letter prefix** — unlike
most sibling carriers here, there is no prefix to anchor on, so this pattern
is necessarily broader than the ones the other carriers' recipes use. It can
match an unrelated 14-digit number in the same mail (an invoice number, say).
The keyword gate and the AI fallback are the safety net for that; **scoping
the IMAP search to Nova Poshta's own sender is the real fix** — see
[Scope & privacy](#scope--privacy) above.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\d{14}')` silently becomes a broken pattern (`\d` is not a recognised string escape, and behaviour is undefined across Jinja versions) — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9])`/`(?![0-9])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you change it.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@novaposhta.ua","subject":"Your parcel is on its way",
       "text":"TTN: 20450000000001","initial":true,"folder":"INBOX","username":"test"}'
```

Then `nova_post.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
