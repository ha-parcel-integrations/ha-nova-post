"""Sensor platform for the Nova Post parcel tracker integration."""
...
from .coordinator import NovaPostCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: NovaPostConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Nova Post sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        NovaPostIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        NovaPostAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            NovaPostParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(NovaPostNextDeliverySensor(coordinator, entry))
    entities.append(NovaPostDeliveredParcelsSensor(coordinator, entry))
    entities.append(NovaPostLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class NovaPostAwaitingPickupSensor(
    CoordinatorEntity[NovaPostCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: NovaPostCoordinator, entry: ConfigEntry) -> None:
...
class NovaPostIncomingParcelsSensor(
    CoordinatorEntity[NovaPostCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: NovaPostCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                NovaPostParcelSensor(self.coordinator, self._entry, barcode)
...
class NovaPostParcelSensor(CoordinatorEntity[NovaPostCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Nova Post parcel."""
    ...
    def __init__(
        self, coordinator: NovaPostCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class NovaPostNextDeliverySensor(
    CoordinatorEntity[NovaPostCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: NovaPostCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class NovaPostDeliveredParcelsSensor(
    CoordinatorEntity[NovaPostCoordinator], SensorEntity
):
    """Recently delivered tracked Nova Post parcels."""
...
class NovaPostLastUpdateSensor(
    CoordinatorEntity[NovaPostCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Nova Post was last polled successfully."""
