"""Tests for Nova Post sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.nova_post.const import ParcelStatus
from custom_components.nova_post.sensor import (
    NovaPostAwaitingPickupSensor,
    NovaPostDeliveredParcelsSensor,
    NovaPostIncomingParcelsSensor,
    NovaPostLastUpdateSensor,
    NovaPostNextDeliverySensor,
    NovaPostParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = NovaPostIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_awaiting_pickup_counts_only_ready_pickup_point_parcels():
    ...
    sensor = NovaPostAwaitingPickupSensor(
...
def test_awaiting_pickup_zero_when_no_parcels():
    sensor = NovaPostAwaitingPickupSensor(_coordinator([]), _entry())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = NovaPostParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = NovaPostParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = NovaPostNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = NovaPostNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = NovaPostDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = NovaPostLastUpdateSensor(coordinator, _entry())
